...
public class Combat {
	...
	public void startCombat() {
		...
		while(player.getCurrentHP() > 0 && enemy.getCurrentHP() > 0) { //TODO boolean isDead
			...
			if(player.getCurrentHP() <= 0 && enemy.getCurrentHP() <= 0) System.out.println("Los dos habeis muerto a la vez..., un señor que pasaba por ahi te robo el loot y te saqueo un poquito mucho, pero como estas muerto te da igual"); //TODO Remove prints, return winner //TODO Nest this if?????
			else if(player.getCurrentHP() <= 0) System.out.println("Has muerto...Try again, insert coin");
			else if(enemy.getCurrentHP() <= 0) System.out.println("Has ganado, pero esto solo es el principio, MUA HAHA HAHA");
